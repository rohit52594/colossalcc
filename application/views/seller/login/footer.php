    ...
    ?>
    var mainToken = localStorage.getItem('seller-access-token')
    $.ajax({
        type: 'POST',
        data: {
            token: mainToken
        },
        url: '<?php echo base_url("seller/authentication/passuser"); ?>',
        dataType: 'text',
        success: function(data) {
            console.log('autologin')
            if (data == 'success')
                window.location.assign('<?php echo base_url('seller/dashboard'); ?>')
        }
    });
    <?php #} 
    ?>
