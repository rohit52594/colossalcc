...
?>
                    ...
                    <div class="input-group">
                        ...
                        <select name="district" id="district" class="form-control" required onchange="getPincodes(this, this.value)">
                            ...
                            <?php foreach ($DISTRICTS as $key) { ?>
                                <option <?php echo $key->district == $DELIVERER_DETAILS[0]->district ? 'selected' : ''; ?> value="<?php echo $key->district; ?>"><?php echo $key->district; ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                        ...
                        <div id="pincodesDiv">
                            <select class="form-control" name="pincode" id="pincode" required>
                                <?php foreach ($PINCODES as $key) { ?>
                                    <option <?php echo $key->pincode == $DELIVERER_DETAILS[0]->locator_pin ? 'selected' : ''; ?> value="<?php echo $key->pincode; ?>"><?php echo $key->pincode; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
    ...
    function checkPassword(val, elem) { if (val == $('#password').val()) { return true; } else { $(elem).val(''); alert('Passwords not matching'); $(elem).focus();} }
